...
def search():
    ...
    reorder_result_60 = consider_pr_pv(last_60, pagerank_w=0.6, pageview_w=0.4, pagerank_dict=app.pagerank,
                                       pageview_dict=app.wid2pv)

    # add titles and combine
    res = [(int(id), app.id_title[id]) for id, score in reorder_result_40]
    res += [(int(id), app.id_title[id]) for id, score in reorder_result_60]
    # END SOLUTION
    return jsonify(res)
...
def consider_pr_pv(docs, pagerank_w , pageview_w , pagerank_dict , pageview_dict):
    sorted_by_weigts = {}
    pr_max, pr_min, pr_mean = (max(pagerank_dict), min(pagerank_dict), np.mean(list(pagerank_dict.values())))
    pv_max, pv_min, pv_mean =(max(pageview_dict), min(pageview_dict), np.mean(list(pageview_dict.values())))
    for doc_id in docs:
        # normalize by minmax
